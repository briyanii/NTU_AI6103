...
def sample_anchors(anchors, gt, lo_th, hi_th, sample_size):
	...
	case_0 = ious.argmax(dim=0)
	case_1 = (ious > hi_th).any(dim=1)
	case_2 = (ious < lo_th).all(dim=1)
	labels = torch.where(case_2, -1, 0)
	...
	# sample up to n//2
	pos_idx = torch.randperm(n_pos)[:pos_sample_size]
	pos_idx = torch.where(labels==+1)[0][pos_idx]
	pos_sample_size = pos_idx.size(0)
	# sample remaining to make up to n
	neg_idx = torch.randperm(n_neg)[:sample_size - pos_sample_size]
	...
	sampled_labels = torch.where(sampled_labels==1, 1, 0)
...
def get_anchor_origins(ft_w, ft_h, stride_len):
	...
	origin_x, origin_y = torch.meshgrid(origin_x, origin_y)
...
def parameterize_bbox(bbox, anchor):
	...
	tw = torch.log(bbox[:, 2] / anchor_w)
	th = torch.log(bbox[:, 3] / anchor_h)
